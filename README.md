...
| `ALLOW_PASSWORD_RESET` | Off unless set to exactly `true`. Emails a single-use reset link, valid for 60 minutes, to the address on file. Also requires a provider key, a configured sender, `DEMO_MODE` off, and a real `PUBLIC_BASE_URL` — if any is missing the feature stays off and the boot log says which. Admins can always reset a password from Access. |
